...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password === "12345") {
      ...
      document.cookie = "auth-session=true; path=/; max-age=86400; SameSite=Lax"; // 24 hours
      router.push("/");
      router.refresh();
    } else {
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col justify-center py-12 sm:px-6 lg:px-8 transition-colors duration-200">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        ...
        <p className="mt-2 text-sm text-slate-600 dark:text-slate-400 font-medium">
          Taller Mecánico • Panel de Control
        </p>
      </div>

      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white dark:bg-slate-900 py-8 px-4 shadow-xl shadow-slate-200/50 dark:shadow-none sm:rounded-2xl sm:px-10 border border-slate-200 dark:border-slate-800 transition-colors">
            ...
            <div>
              ...
              <div className="mt-1 relative">
                ...
                <input
                  ...
                  disabled
                  value="Administrador"
                  className="block w-full pl-10 pr-3 py-2 border border-slate-300 dark:border-slate-700 rounded-lg leading-5 bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 sm:text-sm focus:outline-none transition-colors"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="mt-1 relative">
                ...
                <input
                  ...
                  className="block w-full pl-10 pr-3 py-2 border border-slate-300 dark:border-slate-700 rounded-lg leading-5 bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 sm:text-sm transition-all"
                  placeholder="•••••"
                />
              </div>
            </div>
          ...
          <div className="mt-6 text-center">
            <p className="text-xs text-slate-500 dark:text-slate-500">
              © {new Date().getFullYear()} SmartGarage Software de Gestión.
            </p>
          </div>
        </div>
      </div>
